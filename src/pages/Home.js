...
const Home = () => {
  ...
  const getArtistTracks = async (access_token, artist_id) => {
    try {
      const response = await axios.get(`https://api.spotify.com/v1/artists/${artist_id}/top-tracks?market=SG`, {
        headers: {
          'Authorization': `Bearer ${access_token}`
        }
      });
    ...
    <div className='container'>
      <form>
        ...
        {error ? <div className='input-field'>
          <label className='error'>Please key in a valid artist ID.</label>
        </div> : null}
